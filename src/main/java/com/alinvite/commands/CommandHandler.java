...
public class CommandHandler implements CommandExecutor, TabCompleter {
    ...
    private void handleAdmin(CommandSender sender, String[] args) {
        ...
        switch (adminSub) {
            ...
            case "contrib" -> {
                if (args.length < 3) {
                    sender.sendMessage("&6===== 贡献返点管理 ======");
                    sender.sendMessage("&e/alinvite admin contrib <玩家> &7- 查看贡献返点");
                    sender.sendMessage("&e/alinvite admin contrib add <玩家> <金额> &7- 增加贡献返点");
                    sender.sendMessage("&e/alinvite admin contrib set <玩家> <金额> &7- 设置贡献返点");
                    sender.sendMessage("&e/alinvite admin contrib deduct <玩家> <金额> &7- 扣除贡献返点");
                    sender.sendMessage("&e/alinvite admin contrib clear <玩家> &7- 清空贡献返点");
                    sender.sendMessage("&e/alinvite admin contrib h <玩家> <金额> &7- 兑换贡献返点");
                    sender.sendMessage("&6=====================");
                    return;
                }
                ...
                switch (action) {
                    case "add" -> {
                        if (args.length < 5) {
                            sender.sendMessage("用法: /alinvite admin contrib add <玩家> <金额>");
                            return;
                        }
                        ...
                        plugin.getDatabaseManager().addContributionAmount(target.getUniqueId(), addAmount).thenAccept(success -> {
                            if (success) {
                                String message = "§a成功为玩家 " + target.getName() + " 增加 " + addAmount + " 贡献返点";
                                sender.sendMessage(message);
                                
                                // 发送消息给玩家
                                String playerMsg = "§a您的贡献返点增加了 " + addAmount + " 点券";
                    ...
                    case "set" -> {
                        if (args.length < 5) {
                            sender.sendMessage("用法: /alinvite admin contrib set <玩家> <金额>");
                            return;
                        }
                        ...
                        plugin.getDatabaseManager().setContributionAmount(target.getUniqueId(), setAmount).thenAccept(success -> {
                            if (success) {
                                String message = "§a成功将玩家 " + target.getName() + " 的贡献返点设置为 " + setAmount;
                                sender.sendMessage(message);
                                
                                // 发送消息给玩家
                                String playerMsg = "§a您的贡献返点已被设置为 " + setAmount + " 点券";
                                target.sendMessage(playerMsg);
                            } else {
                                sender.sendMessage("§c操作失败");
                            }
                    ...
                    case "deduct" -> {
                        if (args.length < 5) {
                            sender.sendMessage("用法: /alinvite admin contrib deduct <玩家> <金额>");
                            return;
                        }
                        ...
                        plugin.getDatabaseManager().deductContributionAmount(target.getUniqueId(), deductAmount).thenAccept(success -> {
                            if (success) {
                                ...
                                sender.sendMessage("§c扣除失败: 玩家贡献返点余额不足");
                            }
                        });
                    }
                    case "clear" -> {
                        if (args.length < 4) {
                            sender.sendMessage("用法: /alinvite admin contrib clear <玩家>");
                            return;
                        }
                        ...
                        plugin.getDatabaseManager().clearContributionAmount(target.getUniqueId()).thenAccept(success -> {
                            if (success) {
                                String message = "§a成功清空玩家 " + target.getName() + " 的贡献返点";
                                sender.sendMessage(message);
                                
                                // 发送消息给玩家
                                String playerMsg = "§a您的贡献返点已被清空";
                                target.sendMessage(playerMsg);
                            } else {
                                sender.sendMessage("§c操作失败");
                            }
                    ...
                    case "h" -> {
                        // 兑换贡献返点为点券
                        if (args.length < 5) {
                            sender.sendMessage("用法: /alinvite admin contrib h <玩家> <金额>");
                            return;
                        }
                            ...
                            if (success) {
                                ...
                                SchedulerUtils.runTask(plugin, () -> {
                                    ...
                                    if (pointsSuccess) {
                                        ...
                                        String playerMsg = "§c贡献返点兑换失败，请联系管理员";
                                        target.sendMessage(playerMsg);
                                    }
                                });
                            } else {
                                sender.sendMessage("§c兑换失败: 玩家贡献返点余额不足");
                            }
                        });
                    }
                    default -> {
                        ...
                        // 查询贡献返点
                        plugin.getDatabaseManager().getContributionAmount(target.getUniqueId()).thenAccept(amount -> {
                            String message = "§a玩家 " + target.getName() + " 的贡献返点余额: §e" + amount + " §a点券";
                            sender.sendMessage(message);
                        });
                    }
                }
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        ...
        if (args.length == 3 && (args[1].equalsIgnoreCase("reset") || args[1].equalsIgnoreCase("clearcode") || args[1].equalsIgnoreCase("checkgroup") || args[1].equalsIgnoreCase("contrib"))) {
            return filterByInput(
                Bukkit.getOnlinePlayers().stream().map(Player::getName).collect(Collectors.toList()),
                args[2]
            );
        ...
        if (args.length == 4 && args[1].equalsIgnoreCase("contrib")) {
            return filterByInput(Arrays.asList("add", "set", "deduct", "clear", "h"), args[3]);
        }
