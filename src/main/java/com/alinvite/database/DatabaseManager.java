...
public class DatabaseManager {
    ...
    public CompletableFuture<Void> resetPlayerData(UUID uuid) {
        return CompletableFuture.runAsync(() -> {
            ...
            // 重置玩家数据但保留邀请码
            String sql3 = "UPDATE " + tablePrefix + "players SET total_invites = 0, claimed_milestones = '[]', announced_milestones = '[]', gift_id = NULL, purchased_gifts = '[]', money = 0, contribution_amount = 0, last_permission_group = NULL WHERE uuid = ?";
            try (Connection conn = getConnection();
