...
public class LeaderboardManager {
    ...
    private void init() {
        ...
        // 启动定时更新任务（默认60秒）
        SchedulerUtils.runTaskTimer(plugin, this::updateAllLeaderboards, 0, 60 * 20L);
    }
    ...
    private String getLeaderboardSql(LeaderboardType type) {
        ...
        switch (type) {
            case MILESTONE:
                return "SELECT uuid, name, total_invites as value FROM " + tablePrefix + "players WHERE total_invites > 0 ORDER BY total_invites DESC LIMIT 100";
            case CONTRIBUTION:
                return "SELECT uuid, name, contribution_amount as value FROM " + tablePrefix + "players WHERE contribution_amount > 0 ORDER BY contribution_amount DESC LIMIT 100";
            case POINTS_REBATE:
                return "SELECT uuid, name, total_rebate_points as value FROM " + tablePrefix + "players WHERE total_rebate_points > 0 ORDER BY total_rebate_points DESC LIMIT 100";
            default:
                return null;
        }
    }
